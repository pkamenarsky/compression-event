...
//
// Three stages of the screen pass in `screen.ts`: a 4x4 threshold, for any
// stage after it that lays a pattern down in screen pixels; the walls' nudge,
// which is that threshold over whatever share of a pixel is wall and is what
// keeps a large flat surface from banding; and the quantising itself.
// -----------------------------------------------------------------------------

import * as THREE from 'three';
import type { Stage } from './screen';

/** The 4x4 threshold, `bayerDither(pixel)`, for the stages after it. */
export const bayer: Stage = {
  glsl: /* glsl */ `
 ...
 * clamped where the target would once have clamped it. Needs `Texel` and
 * `bayerDither` ahead of it; `pixel` is whole pixels.
 */
