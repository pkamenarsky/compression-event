...
// The sky
//
// A night over the level, in one bit: white on black and nothing between.
// Stars, a faint nebula along a band of the sky, and the whole of it wheeling
// slowly about a tilted pole while the nebula works on itself underneath.
//
...
//   a star, a fraction for the nebula, 0 for nothing — and the sky's mark.
// - **the stage**, in the screen pass: where the mark is, the density against
//   the 8x8 threshold, after the warp, in the pixels it is seen in. The nebula
//   comes out as a scatter of dots thinning towards its edges, and nothing
//   the quantise does to the rest of the picture reaches it.
//
...
// past it is nothing.
//
// Stars are sized in pixels rather than in sky, so a star is a dot or two
// however wide the view: a cell of a grid on each face of a cube, a star
// somewhere inside the cell and well clear of its edges, lit where the pixel is
// within a pixel or so of it.
// -----------------------------------------------------------------------------
...
const fragmentShader = /* glsl */ `
  ...
  // How much of the sky here is nebula, 0 to 1 before uNebula: patchy cloud
  // folded over itself, heaviest along the band and slowly reworking.
  float nebula(vec3 d, float t) {
    ...
    return smoothstep(0.42, 0.78, n) * mix(0.25, 1.0, band);
  }

  // 1 where a star is, 0 elsewhere. \`px\` is a pixel's width in the direction's
  // own terms, off the direction's derivatives, which unlike a face's
  // coordinates do not jump at the cube's seams.
  float star(vec3 d, float px, float t) {
    ...
    float phase = hash13(vec3(cell, face) + 11.0) * 6.2832;
    float rate = 0.4 + 1.6 * h.z;
    r *= 1.0 - uTwinkle * step(0.6, h.z) * (0.5 + 0.5 * sin(t * rate + phase));

    vec2 at = cell + 0.25 + 0.5 * h.yz;
    float size = px * CELLS * 0.5 / major;

    return step(length(p - at), r * size);
  }
  ...
  void main() {
    ...
    float px = length(fwidth(d)) * 0.7071;
    float above = smoothstep(-0.02, 0.2, seen.y);

    float density = uNebula * nebula(d, uTime);

    density = max(density, star(d, px, uTime));

    fragColor = vec4(vec3(density * above), 1.0);
    marks = skyMark();
  }
`;
...
export function sky(): Sky {
  ...
  return {
    ...
    configure({ sky: s }: RenderConfig): void {
      ...
      material.uniforms.uNebula.value = s.nebula;
