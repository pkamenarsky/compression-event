 ...
 * by the opening while the polygons it resolves to round by `arcsWith` cannot
 * make law 3 green however right either of them is on its own.
 */
function folding(
  source: Ring,
  rings: readonly number[],
  ...
  rows.forEach((row, i) => {
    ...
    for (const c of row) {
      ...
      if (!kept.has(e)) list.push(...c.ops.map(o => carriedKey(made++, c.source.ref, o)));
        ...
        list.push({ ...carriedKey(e.id, c.source.ref, c.ops[0]), times });
      }
    }
...
export function copied(world: World, v: KeyframeId, ids: readonly Id[]): Clipping[] {
  ...
  const timed = (id: Id, outermost: boolean): { time: Timed, corners: ReadonlyMap<VertexId, Point> } => {
    ...
    const carrying = (p: Pick<Playing, 'key' | 'ref' | 'step' | 'by' | 'stand'>): RigKey => ({
      ...p.key,
    ...
    const cornering = (keys: KeyRig, k: KeyframeId): RigKey[] => {
      ...
      return [...keys.keys].flatMap(([w, list]) => {
        ...
        return list.flatMap(key => {
          ...
          const step = counted1(key, j, i) - 1;

          return step > counted1(key, j, i - 1) - 1 ? [carrying({ key, ref: key.ref, step })] : [];
        });
      });
    };
    ...
    const running = stand !== null
      ? []
      : [...playingAt(src, id, v).flatMap(p => (p.at === v ? [] : [carrying(p)])), ...cornering(rig, v)];
    ...
    for (let i = at + 1; i < n; i++) {
      ...
      const steps: RigKey[] = stand === null || i > at + 1
        ? []
        : [...playingAt(before, id, k).map(carrying), ...cornering(keyRigOf(before, id), k)];
...
function written(
  ...
  for (const [offset, list] of clip.keys) {
    ...
    // What the copy keyframe did plays over the stand.
    keys.set(k, [...(keys.get(k) ?? []), ...list.map(e => ({ ...rekeyed(e), id: made++ }))]);
  }
