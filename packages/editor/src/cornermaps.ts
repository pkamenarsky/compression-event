...
//
// The names of the four, what kind of operation each holds, and the two ways
// of going over them. Small, and its own file for one reason: both `rig.ts`
// and `types.ts` have to name the four, and if either owned the list the
// other would read it. `rig.ts` already reads `types.ts` for `enclosing`, so
