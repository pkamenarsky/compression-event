...
describe('gestures', () => {
  ...
  test('a corner nudge is a gesture too, and the key it is written in carries it', () => {
    const { world, ids } = two();
    const corners = world.polygons.get(ids[0])!.points.map(c => c.id);
    const first = step(initialState(world), nudge(world, 0, ids[0], corners.slice(0, 2), { x: 1, y: 0 }));
    const second = step(first, nudge(first.world, 0, ids[0], corners.slice(1, 3), { x: 1, y: 0 }));
    const of = (w: World, i: number) => rigOf(w, ids[0]).nudges.get(corners[i])!.get(0)!.gesture;
    ...
    // so nudging two of them again is that key written again, and the
    // gesture is the key's rather than each corner's. Three corners are moved
