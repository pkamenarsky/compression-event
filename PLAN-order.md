...
The per-corner amounts are what makes this more than a rename. A corner's own
depth, and an edge's own deform options and amplitude, are written today against
the polygon's `VertexId`s in the polygon's own rig and in `cornerEffects`. They
move to the scope that lays that effect, still keyed by the same `VertexId`s.
The names a fold carries are `corner(member, vertex)` however deep they go, so a
scope two levels up can say "this corner, this much" as well as the polygon
could. The editor writes to the nearest scope above the corner whose effect is
of the kind being edited, and makes one when there is none.
...
work in the resolve, and the part to prove first (step 1).
...
in step 6 by the chain that says the same thing.

**1. Prove the names survive a resolve.** Before anything moves: a test in which
...
**2. `Layer`, and `Group.effect`.** The types, and `Effects` read as a chain of
...
**3. Save format, and the conversion.** A version bump in `save.ts` and a step in
...
amounts move from its rig to theirs, and `cornerEffects` moves to the deform
scope's per-corner options. A polygon with effects inside a loose group gets its
scopes inside that group, in its place. The golden files are reconverted, and
`baseline.test.ts` says the drawing did not move.

**4. Polygons lose `Effects`.** The polygon fold is its clean-up and nothing else.
`folding`'s effect half goes, with `roundingOf`, `offsetOf`'s depth handling and
the per-polygon deform options. Per-corner amounts are read from the scope.

**5. Resolve keeps the effect.** `resolveGroup` on a scope with an effect
resolves its members and leaves the scope. `publishing` loses the amounts, the
anchor, reach and key, and the facet fade.

**6. The laws over chains.** `arbKit` becomes a chain of zero to three layers in
...
**7. The bake, one layer per scope.** `Standing` and `Cast` as above, and the
memo per scope. The perf tests say whether the finer memo pays for the extra
stages.

**8. The editor.** The inspector shows a scope's one effect, and "add an effect"
...
two single-member scopes' layers, which keeps their ids and their timelines. A
corner edit goes to the nearest scope of its kind, as above.

**9. Delete what is left.** `Effects`, `Options`' three-in-one shape,
`cornerEffects`, and whatever of `publishing` survived step 5.
...
- **Which slot an effect applies to.** A sealed scope's output is one shape per
  ...
  same. Read `outermostSlot` before step 2 and write down what it does.
- **Where the editor puts a new corner amount.** "The nearest scope of its kind"
  is a guess at what an author wants. The alternative is always a scope of its
  own around the polygon, which is simpler to write and makes more scopes.
...
folded in list order. It keeps polygons' effects, `publishing` and the
per-corner machinery as they are, and it lets the same kind appear twice. It
